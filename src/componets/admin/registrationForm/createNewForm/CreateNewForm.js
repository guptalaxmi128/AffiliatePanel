...
import { addCouponToCourse, getCoupon } from "../../../../actions/addCoupon/addCoupon";
...
const CreateNewForm = () => {
  ...
  const handleCourseChange = (value,options) => {
    const selectedCoursesIds = options.map((option) => option.key);
    setSelectedCourses(selectedCoursesIds);
  };
  ...
  const handleCouponChange = (value, options) => {
    const selectedCouponIds = options.map((option) => option.key);
          ...
          >
            ...
            >
              {coupon?.data?.map((coupon) => (
                <Select.Option key={coupon.id} value={coupon.title}>
                  {coupon.couponName}
                </Select.Option>
              ))}
            </Select>
